// forked from design-sync lib/source-kit.mjs - honor componentSrcMap null-exclusions in the synth-entry file set (app package: recovery-card.tsx drags in env/OIDC and throws at bundle load)
...
export async function resolvePackage(ctx) {
  ...
  if (!entry) {
    ...
    const excludedKebabs = Object.entries(srcMap)
      .filter(([, v]) => v === null)
      .map(([k]) => k.replace(/([a-z0-9])([A-Z])/g, '$1-$2').toLowerCase());
    const excludedRx = excludedKebabs.length
      ? new RegExp(`(?:^|/)(?:${excludedKebabs.join('|')})\\.(tsx|jsx)$`, 'i')
      : null;
    const comps = srcFiles.filter((p) => SRC_IMPL_RX.test(p) && !NON_IMPL_RX.test(p) && !(excludedRx && excludedRx.test(p)));
