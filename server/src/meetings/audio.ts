...
import { seqFromChunkKey } from "../recording/keys.js";
import type { ByteRange, StoredObject } from "../recording/types.js";

/**
 * Playback assembly.
 *
 * The recording is stored as one object per chunk (ADR-002), so playback concatenates them back
 * into the original container stream. Nothing is ever written back: assembling on read keeps the
 * chunk objects the single copy of the audio, which is also what makes the deletion cascade of
 * ADR-001 a simple prefix removal.
 */
...
/**
 * Orders the chunk objects of a session by sequence number and computes their stream offsets.
 *
 * Ordering comes from the sequence number parsed out of the key rather than from the listing
 * order. Lexicographic order happens to agree because the sequence is zero-padded, but relying on
 * that would make the audio silently depend on a storage implementation detail.
 */
export function audioLayout(objects: readonly StoredObject[]): AudioLayout {
